...
| `VK_APP_SECRET` | защищённый ключ: им проверяется подпись параметров запуска VK. Можно перечислить несколько через запятую — пригодится при переезде между приложениями |
| `VK_ID_ENABLED` | `1` включает «Войти через ВКонтакте» на сайте. Без переменной кнопки нет: для входа нужен доверенный redirect URL в настройках приложения VK |
